...
!#ifdef USE_ICE
    ! importable field: sea_surface_salinity from ice
    call NUOPC_Advertise(importState, &
...
!    call ESMF_TimeIntervalSet(stabilityTimeStep, m=10, rc=rc) ! 10 minute steps
    call ESMF_TimeIntervalSet(stabilityTimeStep, m=60, rc=rc) ! 60 minute steps0
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
