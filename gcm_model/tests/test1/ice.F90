...
!      100._ESMF_KIND_R8, 200._ESMF_KIND_R8, 100, 20, rc)
  ...
  subroutine xSetClock(model, rc)
    ...
    call ESMF_TimeIntervalSet(stabilityTimeStep, m=60, rc=rc) ! 60 minute steps
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine CheckImport(model, rc)
    ...
    do i=1, size(fieldList)
      ...
      if (timeCheck) then
        ...
        call ESMF_LogWrite("ICE: detected import field at invalidTime", &
